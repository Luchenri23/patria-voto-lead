          ...
          <div className="hidden lg:flex items-center gap-4">
            <div className="flex items-center gap-2">
              {socialLinks.map(({ icon: Icon, href, label }) => (
                <a key={label} href={href} className={`p-2 rounded-full transition-colors ${isScrolled ? "text-muted-foreground hover:text-secondary hover:bg-secondary/10" : "text-primary-foreground/70 hover:text-primary-foreground hover:bg-primary-foreground/10"}`} aria-label={label} target="_blank" rel="noopener noreferrer">
                  <Icon className="w-4 h-4" />
                </a>
              ))}
              <a href={header?.tiktok_url || "#"} className={`p-2 rounded-full transition-colors ${isScrolled ? "text-muted-foreground hover:text-secondary hover:bg-secondary/10" : "text-primary-foreground/70 hover:text-primary-foreground hover:bg-primary-foreground/10"}`} aria-label="TikTok" target="_blank" rel="noopener noreferrer">
          ...
          <nav className="lg:hidden mt-4 pb-4 border-t border-border/20 pt-4 animate-fade-in-up bg-background rounded-lg p-4 shadow-lg">
            <div className="flex flex-col gap-3">
              ...
              <div className="flex items-center gap-3 py-2">
                <a href={header?.instagram_url || "#"} className="text-muted-foreground hover:text-secondary"><Instagram className="w-5 h-5" /></a>
                <a href={header?.facebook_url || "#"} className="text-muted-foreground hover:text-secondary"><Facebook className="w-5 h-5" /></a>
                <a href={header?.twitter_url || "#"} className="text-muted-foreground hover:text-secondary"><Twitter className="w-5 h-5" /></a>
                <a href={header?.tiktok_url || "#"} className="text-muted-foreground hover:text-secondary"><TikTokIcon className="w-5 h-5" /></a>
              </div>
