    ...
    <section className="py-20 bg-muted">
      <div className="container mx-auto px-4">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="text-center mb-12">
          ...
          <p className="text-muted-foreground max-w-xl mx-auto">Fique por dentro de tudo que acontece! Siga nossas redes sociais e acompanhe de perto as ações e novidades.</p>
        </motion.div>
...
interface SocialPostProps {
  post: { id: string; post_url: string; image_url: string | null; caption: string | null; platform: string; post_type: string };
  index: number;
  aspectClass: string;
}

const SocialPost = ({ post, index, aspectClass }: SocialPostProps) => (
